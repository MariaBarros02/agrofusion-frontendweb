...
import { getDisriegoServiceToken } from "./disriegoTokenManager";
...
export const authPrivateAxios = axios.create({
    ...
    baseURL: import.meta.env.VITE_API_DISRIEGO_URL,
...
export const applyServiceAuthInterceptor = (api: AxiosInstance) => {
  api.interceptors.request.use(async (config) => {
    const token = await getDisriegoServiceToken(useAuthStore.getState().email ?? "");
