...
export const getUserByEmailService = async (email:string) => {
  const {data} = await  authApiDisriego.getUserByEmail(email);
  return data;
}
...
export const changeStateUserService = async (user_id: number, new_status: number) => {
  const {data} = await  authApiDisriego.changeUserStatus(user_id, new_status);
  return data;
}
...
export const editUserByAdminService = async (payload: ExternalUser, id: number) => {
  const {data} = await  authApiDisriego.editUserByAdmin(payload, id);
  return data;
}
...
export const changePasswordService = async (payload: ChangePasswordRequest, id: number) => {
  const {data} = await  authApiDisriego.changePasswordUser(payload, id);
  return data;
}